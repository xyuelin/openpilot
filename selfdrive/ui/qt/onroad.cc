...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if ((experimentalMode || scene.acceleration_path) && !((currentHolidayTheme != 0 || customColors != 0) && abs(currentAcceleration) < 0.25)) {
...
void DistanceButton::paintEvent(QPaintEvent *event) {
  ...
  if (textOpacity != 0.0) {
    ...
    p.setPen(Qt::white);
    QRect textRect(0, 0, width(), height() + 95);
...
void AnnotatedCameraWidget::drawSLCConfirmation(QPainter &p) {
  ...
  p.drawText(leftRect, Qt::AlignCenter | Qt::AlignTop, rightHandDM ? ignoreText : confirmText);
  p.drawText(rightRect, Qt::AlignCenter | Qt::AlignTop, rightHandDM ? confirmText : ignoreText);
