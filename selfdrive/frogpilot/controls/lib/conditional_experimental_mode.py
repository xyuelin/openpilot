...
from openpilot.selfdrive.frogpilot.controls.lib.frogpilot_functions import CITY_SPEED_LIMIT, PROBABILITY, MovingAverageCalculator
...
class ConditionalExperimentalMode:
  ...
  def update(self, carState, enabled, frogpilotNavigation, modelData, radarState, road_curvature, stop_distance, t_follow, v_ego):
    ...
    condition_met = self.check_conditions(carState, frogpilotNavigation, lead, modelData, stop_distance, v_ego) and enabled
    ...
    self.update_conditions(lead, modelData, road_curvature, stop_distance, t_follow, v_ego, v_lead)
  ...
  def update_conditions(self, lead, modelData, road_curvature, stop_distance, t_follow, v_ego, v_lead):
    self.lead_detection(lead)
    self.lead_slowing_down(lead, stop_distance, v_ego, v_lead)
    self.road_curvature(road_curvature)
    self.slow_lead(lead, t_follow, v_ego)
  ...
  def lead_slowing_down(self, lead, stop_distance, v_ego, v_lead):
    if self.lead_detected:
      lead_close = lead.dRel < CITY_SPEED_LIMIT
  ...
  def slow_lead(self, lead, t_follow, v_ego):
    if self.lead_detected:
